...
%     end
    ...
    subArray(1) = null; %HAD TO CHANGE THIS FOR LEANDRA DATA. PREVIOUSLY NULL==1
    subArray(2) = treat;
    ...
    for k = 1:size(index,1)
        ...
        matchedArray(1).ca.caData(counter) = subArray(1).ca.caData(index(k,1)); %depending on order could be k,1 or k,2
        matchedArray(2).ca.caData(counter) = subArray(2).ca.caData(index(k,2));
        counter = counter +1;
    end
    clear counter k
