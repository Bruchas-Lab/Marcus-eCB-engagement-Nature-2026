%%This script takes bitTIFF files from inscopixProcessorStep1 and runs the
%%initial sections of the CNMF 1P processing script.  It stops the process
%%just before manual scoring is necessicary and saves the workspace as a
%%*.mat file.  Manual scoring is done in the CNMFEprocessorStep2 script.
%%James Hyde 1/16/18
...
%% get the file names and paths for each folder to be processed
while ch == 1
    [FileName, PathName] = uigetfile({'*.tiff'}, 'File Selector', 'MultiSelect', 'on')
    choice = menu('Are there any other folders to add?','Yes','No')
    if isa(FileName, 'char') == 1;
        f{1} = FileName;
        FileName = f;
    end
    
    Files{folderNumber} = FileName;
    Path{folderNumber} = PathName;
...
clear FilesName PathName choice ch f
...
%% choose data
neuron = Sources2D();
nam = get_fullname(FileName{k});          % this demo data is very small, here we just use it as an example
nam = neuron.select_data(nam);  %if nam is [], then select data interactively

%% parameters
% -------------------------    COMPUTATION    -------------------------  %
pars_envs = struct('memory_size_to_use', 8, ...   % GB, memory space you allow to use in MATLAB
    'memory_size_per_patch', 0.4, ...   % GB, space for loading data within one patch
    'patch_dims', [44, 44]);  %GB, patch size
...
ring_radius = 14;  %18 when the ring model used, it is the radius of the ring used in the background model. 15 have been using for a while
%otherwise, it's just the width of the overlapping area
num_neighbors = 50; % number of neighbors for each neuron

% -------------------------      MERGING      -------------------------  %
show_merge = false;  % if true, manually verify the merging step
merge_thr = 0.8;     % thresholds for merging neurons; [spatial overlap ratio, temporal correlation of calcium traces, spike correlation]
...
merge_thr_spatial = [0.8, 0.1, -inf];  % merge components with highly correlated spatial shapes (corr=0.8) and small temporal correlations (corr=0.1) .5 .4

% -------------------------  INITIALIZATION   -------------------------  %
K = [40];             % maximum number of neurons per patch. when K=[], take as many as possible.
...
neuron.remove_false_positives(); %this line sometimes removes real neurons it seems like?
neuron_init = neuron.copy();

%% estimate the background components
neuron.update_background_parallel(use_parallel);

    neuron.update_spatial_parallel(use_parallel); %new
...
neuron.orderROIs('mean');   % order neurons in different ways {'snr', 'decay_time', 'mean', 'circularity'}  new
...
%% Save the Workspace
name = strcat((FileName{k}(1:end-4)), 'cnmfPreProcessed','.mat');
save(name, '-v7.3');

        catch ME
            disp('mistake in file number'); disp(k); disp(FileName);
        end

clearvars -except Files Path j k FileName PathName
    end
end
  clear
